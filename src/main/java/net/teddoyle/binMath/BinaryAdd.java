...
/**
 * Class BinaryAdd
 *     This class does much of the heavly lifting in the program
 *  displaying the GUI and handing user interaction events.
 * 
...
public class BinaryAdd extends Frame implements ActionListener, ItemListener {
	...
	Checkbox addCheckbox, subtractCheckbox;
	CheckboxGroup cbg_op;
	Checkbox signedCheckbox, unsignedCheckbox;
	CheckboxGroup numberTypeCBG;
	int numberType = UNSIGNED;
	TextField[] AValues, BValues, CValues; // Decimal, Hex and Binary
	Label[] bitALabel, bitBLabel, bitCLabel, carryOutLabel;
	
	TextField[] inputBit_A_ValueTextField, inputBit_B_ValueTextField,
	            calcBit_A_ValueTextField, calcBit_B_ValueTextField,
	            resultBit_C_ValueTextField, carryOutTextField;
	TextField carryInTextField;
	TextField N_FlagTextField, Z_FlagTextField, V_FlagTextField, C_FlagTextField;

	ArrowCanvas carryInArrow[];
	ArrowCanvas carryOutArrow[];
	FullAdderCanvas fullAdder[];
	GateCanvas C_outputPathCanvas[];
	...
	GateCanvas B_inputPathCanvas[][];   // first index indicates bit number,
	   ...
	   public BinaryAdd()
	   {
	      ...
	      resultBit_C_ValueTextField = new TextField[NUM_BITS];
	      ...
	      C_outputPathCanvas = new GateCanvas[NUM_BITS];
	  ...
	  public void actionPerformed(ActionEvent event)
	  {
	   ...
	   public void updateValues()
	   {
	      ...
	      String ABitString, BBitString, CBitString, carryInString;
	...
	*/
	      if (ADDITION == currentOperation) ValueC = ValueA + ValueB;
	      else ValueC = ValueA - ValueB;
	      if (UNSIGNED == numberType)
	      {
	          if (ValueC < 0) ValueC = 16 + ValueC;
	          if (ValueC > 15) ValueC %= 16;
	      }
	      else // numberType is SIGNED
	      {
	          while (ValueC < -8) ValueC = 16 + ValueC;
	          while (ValueC > 7) ValueC = ValueC - 16;
	      }
	      ...
	      for (i = 0; i < 3; i++)
	      {
	         ...
	         if ((i > 0) && (ValueC < 0)) value = 16 + ValueC;
	         else value = ValueC;
	         CValues[i].setText(Integer.toString(value, base));
	      }
	      ...
	      for (i = 0, mask = 1; i < NUM_BITS; i++, mask = mask << 1)
	      {
	         ...
	         resultBit_C_ValueTextField[i].setText(bitString[sum]);
	         carryOutTextField[i].setText(bitString[carry]);

	         B_inputPathCanvas[i][1].setInverted(invert);
	      }  // endFor i
	      if (0 == (15 & ValueC) ) Z_FlagTextField.setText("1");
	      ...
	      CBitString = resultBit_C_ValueTextField[NUM_BITS - 1].getText();

	      if (ABitString.equals(BBitString) && !(ABitString.equals(CBitString)) )
	           V_FlagTextField.setText(oneString);
	      else V_FlagTextField.setText(zeroString);

	      if (CBitString.equals(oneString)) N_FlagTextField.setText(oneString);
	   ...
	   {
	      GraphPaperLayout layout = new GraphPaperLayout(new Dimension(36, 23) );
	...
	if ( i > -1)
	 {
	         ...
	         resultBit_C_ValueTextField[i] = new TextField(1);
	         resultBit_C_ValueTextField[i].setEditable(false);

	         c.gridx = LEFT + (3 - i) * 8 + 6;
	         c.gridy = TOP + 19;
	         add(resultBit_C_ValueTextField[i],
	         ...
	         C_outputPathCanvas[i] = new GateCanvas(0);
	         add(C_outputPathCanvas[i], new Rectangle(6 + (3 - i) * 8, 18, 1, 1) );
